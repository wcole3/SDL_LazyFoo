...
public:
    ...
    bool loadFromFileWithPixels(std::string path, bool colorKeying = false, SDL_Color colorKey = {0,0,0});
#ifdef _SDL_TTF_H
...
//method to create a blank texture
bool lTexture::createTexture(int width, int height){
    bool successFlag = true;
    mTexture = SDL_CreateTexture(lRenderer, SDL_PIXELFORMAT_RGB888, SDL_TEXTUREACCESS_STREAMING, width, height);
    if(mTexture == NULL){
...
bool lTexture::loadFromFileWithPixels(std::string path, bool colorKeying, SDL_Color colorKey){

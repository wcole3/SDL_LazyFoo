...
public:
    ...
    void move(float time, lTile* tiles[]);
...
void lRigidDot::move(float timeStep, lTile* tiles[]){
    ...
    //test wall collision
    else if(touchingTileWall(getCollider(), tiles)){
        xCenterPos = oldXPos;
        shiftCollider();
    }
    //do the same for y direction
    yCenterPos += timeStep*yVel;
    shiftCollider();
    //check collisions
    if(yCenterPos - mCollisionCircle.r < 0){
        yCenterPos = mCollisionCircle.r;
